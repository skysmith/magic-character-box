...
- Offers an explicit `pn532-ndef` hosted mode that identifies a Story Sticker
  ...
  the same opaque `sdpk1_...` identity. An exact inactive suffix receives only
  a constant discovery key and reaches the ordinary unknown-tag cue; it cannot
  select audio until authenticated hosted config maps it. UID is only a learned
  local acceleration.
...
mismatched active aliases fail closed. A hashed local UID binding
